 ...
 *
 * On repack we:
 * 1. Update .bun in place via `bunSection.content = ...` — LIEF's Builder
 *    rewrites the file layout from the modified section, keeping the section
 *    at its original vaddr/file offset
 * 2. If the section sits in its own dedicated PT_LOAD (older post-PR#26923
 *    layout), update that segment's fileSize/virtualSize to cover the new
 *    content. If the section lives inside a larger LOAD segment (CC 2.1.133+
 *    Linux: .bun appended to the existing RW data segment), let LIEF rewrite
 *    the surrounding segment's headers from the section delta — no manual
 *    resize. Always touching nextVirtualAddress() instead would move the
 *    section, leaving the original bytes behind in the file (binary grows by
 *    ~.bun-size with every --apply / adhoc-patch, and `strings` finds two
 *    copies of every prompt).
 * 3. Patch BUN_COMPILED.size with the (unchanged) section vaddr so the Bun
...
function repackELFSection(
  ...
  try {
    ...
    // Newer format (CC 2.1.133+ Linux): no dedicated segment — the .bun
    // section sits at the end of the existing RW data LOAD segment. In that
    // case the section content can be updated in place without resizing any
    // segment; LIEF's ELF Builder rewrites the file layout from the section
    // change.
    ...
    // Find the original BUN_COMPILED location by searching for the .bun
    // section's vaddr value at BLOB_HEADER_ALIGNMENT-aligned virtual addresses
    // in the RW LOAD segment. writeBunSection() wrote the vaddr at the
    // ORIGINAL .bun section location (where BUN_COMPILED lives in the base
    // binary's data segment), then relocated the section header to point to
    // the appended data.
    ...
    if (dedicatedSegment) {
      ...
      // LIEF's ELF Builder handles the actual file layout, but we need to
      // tell the segment how big the mapped region should be.
      ...
      // segment in CC 2.1.133+ Linux builds). LIEF's ELF Builder rewrites the
      // segment headers from the section delta, so no manual resize.
      debug(
    ...
    // Patch BUN_COMPILED.size to point to the .bun section vaddr. The vaddr
    // doesn't change (LIEF keeps the section at the same virtual address), so
    // we re-write the same value to ensure it's correct.
